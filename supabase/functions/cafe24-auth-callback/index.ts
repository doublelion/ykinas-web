...
serve(async (req: Request) => {
  try {
    ...
    // 💡 2. 필수 파라미터 추출 (code 변수 선언 누락 방어)
    const code = url.searchParams.get("code");
    const mall_id = url.searchParams.get("mall_id") || url.searchParams.get("state"); 

    if (mall_id === "install") {
      throw new Error("인가 URL의 state 파라미터에 'install' 대신 실제 쇼핑몰 아이디(예: ecudemo389879)를 넣어주세요.");
    }
    ...
    // 7. 성공 HTML 응답
    const successUrl = `https://${mall_id}.cafe24.com/disp/admin/shop1/mode/appstore`; 
