  ...
  try {
    ...
    const injectedScript = `
      (function() {
        ...
        // [1] 로그인 페이지 완벽 격리 (드로어 미생성, 파라미터 리셋)
        if (isLoginPage) {
        ...
        // [2] ★ 복원됨: 글로벌 로그인 링크 인터셉터 (인라인 마크업 의존성 제거)
        // <a> 태그나 클릭 이벤트가 login.html을 향할 때 무조건 가로채서 드로어를 엽니다.
        document.addEventListener('click', function(e) {
          const loginLink = e.target.closest('a[href*="/member/login.html"]');
          if (loginLink && !loginLink.getAttribute('target') && !loginLink.href.includes('noMemberOrder')) {
            e.preventDefault();
            e.stopPropagation();
        ...
        // [3] ★ 복원됨: 비동기 큐(Queue) 패턴이 적용된 YkinasLogin 객체
        // 렌더링 전 클릭 시 에러를 방지하고 DOM 준비 후 안전하게 띄웁니다.
        window.YkinasLogin = {
          open: function() {
        ...
        function initShadowDOM() {
          ...
          if (!document.body) return; // body 파싱 전 실행 방지
