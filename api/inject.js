  ...
  try {
    ...
    const injectedScript = `
      (function() {
        ...
        const wasQueued = window.YkinasLogin && window.YkinasLogin._queued;
        ...
        function initShadowDOM() {
              ...
              <div id="login-panel" class="custom-scrollbar-02">
                ...
                <div class="px-8 sm:px-10 py-16 flex-1 flex flex-col justify-center drawer-content-wrapper">
                  <div id="ui-login-mode" class="fade-in">
                    ...
                    <div class="mt-12 text-center border-t border-gray-100 pt-8"><p class="text-xs text-gray-400 font-light mb-4">비회원으로 주문하셨나요?</p><button type="button" id="btn_go_guest" class="inline-flex items-center justify-center w-full bg-white border border-black text-black py-4 text-sm font-medium tracking-widest hover:bg-black hover:text-white transition-colors duration-300 cursor-pointer">비회원 주문 조회하기</button></div>
                  </div>
                </div>
              </div>
            </div>
          ...
          shadowRoot.querySelector('#btn_go_guest').addEventListener('click', () => {
            window.location.href = skinPrefix + '/member/login.html?noMemberOrder&returnUrl=' + encodeURIComponent('/myshop/order/list.html');
          });
            ...
            if (window.MemberAction && typeof window.MemberAction.snsLogin === 'function') {
              ...
              try {
                if (proxyIframe.contentWindow && typeof proxyIframe.contentWindow.MemberAction.snsLogin === 'function') {
                  proxyIframe.contentWindow.MemberAction.snsLogin(provider, currUrl);
                } else throw new Error();
              } catch (e) {
        ...
        // 스크립트 로드 전 사용자가 클릭하여 큐(Queue)에 등록되었다면 즉시 실행
        if (wasQueued) {
    ...
    return res.status(500).send(`console.error('[YKINAS Core] Execution error.');`);
  }
}